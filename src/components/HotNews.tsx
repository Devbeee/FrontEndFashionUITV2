...
export function HotNews ({ newsItems } : HotNewsProps) {
  return (
    <div className="rounded-lg border border-gray200 p-4 shadow-md mb-5">
      <Title level={5} className="text-base uppercase font-bold mb-2 border-b border-gray200 pb-1">
        <Link to="/blogs/all" title="Tin tức nổi bật" className="">
          <span className='text-blue-cyan hover:text-primary'>Tin tức nổi bật</span>
        </Link>
      </Title>
      <div>
        {newsItems.map((item, index) => (
          <div key={index} className="flex mb-3 border-b border-dashed border-gray200 pb-2">
            ...
            <div className="flex-1">
              ...
              <Paragraph className="text-xs text-left italic text-gray600 mb-0">{item.date}</Paragraph>
            </div>
          </div>
        ))}
      </div>

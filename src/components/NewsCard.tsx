...
export function NewsCard ({ blog } : NewsCardProps) {
    ...
    <div className="w-[17rem] p-0 border-none">
      <div className='flex flex-col items-center justify-center w-full'>
        <div className='w-full h-44 object-cover z-0'>
          <Link to={`/blog/${blog.slug}`} title={blog.title} className='z-0'>
              <img
        ...
        <div className="h-44 overflow-hidden py-2 px-3 -mt-8 border rounded-md shadow-md z-50 w-11/12 bg-white-primary">
          <Title level={5} className="text-center ">
            <Link to={`/blog/${blog.slug}`} title={blog.title} className='font-bold line-clamp-2'>
              <span className='text-blue-cyan hover:text-primary transition duration-300 ease-in-out'>{blog.title}</span>
            </Link>
          </Title>
          <Paragraph className="text-xs text-gray600 text-center my-2 relative">
            <span className="bg-blue-cyan border border-white-primary text-white-primary px-2 py-[1px] rounded-full z-10 relative">
          ...
          <Paragraph className="text-center text-gray600 line-clamp-3">
            {blog.shortdesc}
          </Paragraph>
        </div>
      </div>
